...
export default class JWTUtils {
    ...
    static generateAndSendTokens(data, res) {
        ...
        res.cookie('token', token, { httpOnly: false, path: '/', maxAge: 2 * 60 * 60 * 1000, secure: true, sameSite: 'none' });
        res.cookie('refreshToken', refreshToken, { httpOnly: false, path: '/', maxAge: 24 * 60 * 60 * 1000, secure: true, sameSite: 'none' });
        return {token: token, refreshToken: refreshToken}
    }
